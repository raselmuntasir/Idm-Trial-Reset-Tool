...
Func Register($FName = "Rasel muntasir")
	Reset()
	autorun('off')
...
Func DeepCleanup()
	...
	RegDelete("HKEY_LOCAL_MACHINE\SOFTWARE\Internet Download Manager")
	RegDelete("HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Internet Download Manager")
	clearTemp()
EndFunc   ;==>DeepCleanup
...
Func RegSearch($value = '')
	...
	Local $findOutput = _getDOSOutput('findstr /N /I ' & $value & ' ' & @TempDir & "\reg_query.tmp")
	If $findOutput <> '' Then
		Local $find = StringSplit($findOutput, ':')[1]
		$find = Number($find) - 1
